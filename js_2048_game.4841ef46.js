...
startButton.addEventListener('click', ()=>{
    const currentStatus = game.getStatus();
    if (currentStatus === 'idle') game.start();
    else if (currentStatus === 'win' || currentStatus === 'lose') game.restart();
...
},{"./modules/Game.class.js":"3Rljr","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"3Rljr":[function(require,module,exports,__globalThis) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
...
class Game {
    constructor(initialState = [
        ...
        this.status = 'idle';
        const startButton = document.querySelector('.button.start');
        if (startButton) startButton.classList.add('restart');
    }
    getState() {
        return this.board;
    }
    ...
    move(direction) {
        ...
        if (direction === 'left' || direction === 'right') newBoard = newBoard.map((row)=>{
            const { shiftedRow, scoreIncrease } = this.shiftRow(row, direction === 'right');
            ...
            newBoard = newBoard.map((row)=>{
                const { shiftedRow, scoreIncrease } = this.shiftRow(row, direction === 'down');
                totalScoreIncrease += scoreIncrease;
                return shiftedRow;
            });
            newBoard = this.transpose(newBoard);
        ...
        if (boardChanged) this.score += totalScoreIncrease;
    }
    shiftRow(row, reverse) {
        let shiftedRow = row.filter((cell)=>cell !== 0);
        if (reverse) shiftedRow.reverse();
        let scoreIncrease = 0;
        for(let i = 0; i < shiftedRow.length - 1; i++)if (shiftedRow[i] === shiftedRow[i + 1]) {
            shiftedRow[i] *= 2;
            scoreIncrease += shiftedRow[i];
            shiftedRow[i + 1] = 0;
        }
        shiftedRow = shiftedRow.filter((cell)=>cell !== 0);
        while(shiftedRow.length < 4)shiftedRow.push(0);
        if (reverse) shiftedRow.reverse();
        return {
            shiftedRow,
            scoreIncrease
